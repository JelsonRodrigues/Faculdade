...
// Funções auxiliares
void imprimirResultados(Resultado *res);
...
int main(int argc, char **argv) {
    ...
    if (argc > 2) {
        if (argv[2][0] == '-') {
            passadas = atoi(argv[2] + sizeof(char));
        }
        else {
            passadas = atoi(argv[2]);
        }
    }
    ...
    if (passadas == 0) passadas = 5;

    Resultado resultados = criaResultado();
    ...
    printf("\nColetando dados para vetor com tamanho: %u", tamanho);
    for (uint i = 0; i < passadas; i++) {
        ...
        gettimeofday(&fim, 0);
        printf("Ordenado corretamente? %s\n", (estaOrdenado(vetor_ordenar, tamanho) ? "SIM" : "NAO"));
...
void imprimirResultados(Resultado *res) {
    printf("\nRESULTADOS\n");
    time_t media_segundos;
    suseconds_t media_micro_segundos;
    ...
    printf("Media QuickSort: \t%02ld:%02ld:%03ld\n", (media_segundos / 60), (media_segundos % 60), (media_micro_segundos / 1000));
    ...
    printf("Media MergeSort: \t%02ld:%02ld:%03ld\n", (media_segundos / 60), (media_segundos % 60), (media_micro_segundos / 1000));
    ...
    printf("Media InsertionSort: \t%02ld:%02ld:%03ld\n", (media_segundos / 60), (media_segundos % 60), (media_micro_segundos / 1000));
    ...
    printf("Media SelectionSort: \t%02ld:%02ld:%03ld\n", (media_segundos / 60), (media_segundos % 60), (media_micro_segundos / 1000));
    ...
    printf("Media BubbleSort: \t%02ld:%02ld:%03ld\n", (media_segundos / 60), (media_segundos % 60), (media_micro_segundos / 1000));
...
bool estaOrdenado(const int *vetor, const uint numero_elementos){
    if (vetor == NULL || numero_elementos <= 1) return true;
